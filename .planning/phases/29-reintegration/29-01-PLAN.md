...
must_haves:
  truths:
    - "User can evaluate custom scripts and decide: add to chezmoi, keep local, or discard"
    - "User can reintegrate machine-specific env vars via chezmoi templates or a local override file"
    - "User can merge valuable config edits from drifted files into chezmoi source"
  artifacts:
    - path: ".planning/phases/29-reintegration/RUNBOOK-04-reintegration.md"
      provides: "Step-by-step reintegration procedures for all 3 REINT requirements"
      contains: "## Procedure 1"
  key_links: []
---

<objective>
Create the Reintegration runbook: a step-by-step guide the user follows on the client Mac to evaluate and merge local customisations captured during Phase 26 audit and Phase 28 conflict resolution back into the chezmoi-managed dotfiles.

Purpose: After Phase 28 deployed chezmoi-managed files and overwrote local edits (saving them to `~/migration-audit/local-edits/`), the user needs procedures to decide what to do with custom scripts, machine-specific env vars, and valuable config edits. Some belong in chezmoi (templated for client machine_type), some should stay local, and some can be discarded.
Output: RUNBOOK-04-reintegration.md with 3 procedures (one per REINT requirement)
</objective>
...
<task type="auto">
  <name>Task 1: Create reintegration runbook</name>
  <files>.planning/phases/29-reintegration/RUNBOOK-04-reintegration.md</files>
  <action>
Create a markdown runbook at the specified path. The runbook is a set of procedures the user will follow **on the client Mac** to evaluate and merge local customisations into the chezmoi-managed dotfiles. It is NOT automation -- it is a human-readable, step-by-step guide.

Follow the same format established in Phase 26, 27, and 28 runbooks: Purpose, Prerequisites, Output, then numbered Procedures, each with Objective, Steps, Expected output, Troubleshooting, and Output subsections. End with Summary, Verification, and Next Steps.

The runbook must contain these sections:

**Header:**
- Title: "Reintegration Runbook"
...
Expected output: Every script in `untracked-scripts.txt` has been triaged. Scripts added to chezmoi appear in `chezmoi managed`. Local-only scripts are documented. Discarded scripts are removed.
...
Output: All custom scripts triaged. `~/migration-audit/local-scripts-inventory.txt` documents any scripts kept locally.
...
Expected output: Client-specific environment variables are either templated in chezmoi or documented in a local override file. A new shell session loads all variables correctly.
...
Output: Environment variables reintegrated. Critical client-specific variables are either in chezmoi templates or in `~/.zsh.d.local`.
...
Expected output: All valuable local edits merged into chezmoi source. `chezmoi diff --exclude=scripts` produces empty output. The shell behaves as expected with all customisations intact.
...
Output: All drifted config edits reviewed and resolved. chezmoi source is up to date with all valuable local customisations.
...
- Follow the same format as RUNBOOK-01, RUNBOOK-02, and RUNBOOK-03 (Purpose, Prerequisites, Output, Procedures, Summary, Next Steps)
- Code blocks should be bash with comments explaining each step
- Each procedure should reference specific audit artefacts from Phase 26 and Phase 28
- Include decision criteria for each triage step (add to chezmoi / keep local / discard)
- Explain the chezmoi patterns for machine_type conditionals, `.chezmoiignore`, and template conversion
- The user knows their way around chezmoi and the terminal -- explain the decision-making framework, not basic commands
  </action>
  <verify>
Verify the runbook file exists and contains all 3 procedures:
```bash
test -f .planning/phases/29-reintegration/RUNBOOK-04-reintegration.md
grep -c "## Procedure" .planning/phases/29-reintegration/RUNBOOK-04-reintegration.md
# Should output 3
```
  </verify>
  <done>
Runbook file exists with 3 procedures covering REINT-01 through REINT-03. Each procedure has numbered steps, bash commands, decision criteria, expected outputs, troubleshooting, and clear instructions referencing Phase 26 and Phase 28 audit artefacts.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 2: Review reintegration runbook for accuracy</name>
  <files>.planning/phases/29-reintegration/RUNBOOK-04-reintegration.md</files>
  <action>
Review the reintegration runbook for accuracy against the client Mac setup. Verify:
- Procedure 1: Does the script triage workflow match the actual format of `untracked-scripts.txt` from Phase 26? Is `chezmoi add` the right approach for scripts?
- Procedure 2: Is the env var reintegration approach practical? Does the local override file pattern (`~/.zsh.d.local`) fit with the existing shell architecture? Would you prefer a different location?
- Procedure 3: Does the merge workflow cover the scenarios you expect? Is `chezmoi merge` configured on your client Mac, or should the runbook emphasise manual editing?
  </action>
  <what-built>Reintegration runbook with 3 procedures for evaluating and merging local customisations into chezmoi. Covers: custom script triage (add to chezmoi, keep local, or discard), environment variable reintegration (via templates or local override file), and drifted config merge (using chezmoi edit, chezmoi merge, or source replacement).</what-built>
  <how-to-verify>
    1. Open `.planning/phases/29-reintegration/RUNBOOK-04-reintegration.md`
    2. Review Procedure 1: Is the script triage decision framework clear? Are the `chezmoi add` and `.chezmoiignore` instructions correct for managing client-only scripts?
    3. Review Procedure 2: Is the env var classification practical? Does the template conversion workflow (`.zsh` to `.zsh.tmpl`) make sense? Is `~/.zsh.d.local` an acceptable location for a local override file?
    4. Review Procedure 3: Are the three merge approaches (manual edit, chezmoi merge, source overwrite) clear? Do they cover the conflict types you expect from the migration?
    5. Overall: Would you be able to follow this runbook on the client Mac after completing Phase 28 migration?
  </how-to-verify>
  <verify>User has reviewed the runbook and approved or provided feedback for corrections.</verify>
  <done>Reintegration runbook reviewed and approved by user.</done>
...
<verification>
- Runbook file exists at `.planning/phases/29-reintegration/RUNBOOK-04-reintegration.md`
- All 3 REINT requirements are covered (one procedure each)
- Procedure 1 references `~/migration-audit/untracked-scripts.txt` and explains `chezmoi add` with `.chezmoiignore` for client-only scripts
- Procedure 2 provides two approaches: chezmoi template (with machine_type conditional) and local override file
- Procedure 3 references `~/migration-audit/local-edits/` and `dotfiles-drift-diff.txt`, provides three merge approaches
- All procedures include decision criteria (add to chezmoi / keep local / discard)
- Chezmoi patterns are correctly documented (template conversion, `.chezmoiignore` syntax, `chezmoi add`, `chezmoi merge`)
</verification>

<success_criteria>
1. REINT-01: Runbook contains procedure to evaluate custom scripts with triage workflow and chezmoi add instructions
2. REINT-02: Runbook contains procedure to reintegrate env vars via chezmoi templates (machine_type conditionals) or local override file
3. REINT-03: Runbook contains procedure to merge drifted config edits with three approaches (manual edit, chezmoi merge, source overwrite)
4. User has reviewed and approved the runbook for accuracy against their client Mac setup
</success_criteria>

<output>
After completion, create `.planning/phases/29-reintegration/29-01-SUMMARY.md`
</output>
