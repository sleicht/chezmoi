---
description: "MUST use whenever the user mentions GitLab merge requests, MRs, pipelines, CI/CD, GitLab issues, or the glab CLI. Covers listing/viewing/creating/approving/merging MRs, pipeline status, CI logs, retrying or running pipelines, linting .gitlab-ci.yml, and managing GitLab issues. Contains essential glab command syntax — do not run glab without consulting it first. Trigger for: 'merge request', 'MR', 'pipeline', 'CI/CD', 'CI logs', 'GitLab issue', 'glab', '!123'-style references, pipeline URLs, 'my MRs', 'approve MR', 'merge when green', 'CI failed', 'rerun pipeline', 'create MR', 'draft MR', 'squash merge', 'rebase MR', 'MR diff', 'lint CI config'. Also trigger when the user is in a GitLab-hosted repo and asks to 'create a PR' — that maps to a merge request. ANY message containing an MR reference pattern (!digits, e.g. !123) MUST trigger this skill."
model: haiku
---
