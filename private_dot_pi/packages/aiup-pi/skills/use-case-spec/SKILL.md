---
name: aiup-use-case-spec
description: |
  This skill should be used when the user asks to "write a use case", "specify a use case",
  "document system behavior", "define scenarios", "write a functional spec", or mentions
...
Create or update use case specification documents in `docs/use_cases/`. The user provides one or more use case IDs (e.g. UC-001, UC-002).

Each use case describes a complete interaction between an actor and the system to achieve a goal. Write one separate file per use case — never bundle multiple use cases into a single document.
...
See `references/use-case.md` for the document template.
