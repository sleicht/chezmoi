...
function updateStatus(context: { hasUI?: boolean; ui?: { setStatus?(key: string, value: string): void } }): void {
	if (!context.hasUI) return;
	context.ui?.setStatus?.("auto-mode", `auto-mode: ${enabled ? "on" : "off"}`);
}

function notifyStatus(context: { hasUI?: boolean; ui?: { notify(message: string, level?: string): void } }): void {
	if (!context.hasUI) return;
	context.ui?.notify(`auto-mode: ${enabled ? "on" : "off"}`, "info");
}

function setEnabled(value: boolean, context: { hasUI?: boolean; ui?: { notify?(message: string, level?: string): void; setStatus?(key: string, value: string): void } }): void {
	enabled = value;
	updateStatus(context);
	notifyStatus(context as { hasUI?: boolean; ui?: { notify(message: string, level?: string): void } });
}

function handleAutoModeCommand(args: string | undefined, context: { hasUI?: boolean; ui?: { notify?(message: string, level?: string): void; setStatus?(key: string, value: string): void } }): void {
